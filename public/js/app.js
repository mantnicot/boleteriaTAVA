(function () {
  ...
  let verificacionPendingQr = null;
  let verifMediaStream = null;
  let verifScanRaf = null;
  ...
  function setSubBars(view) {
    ...
    const sbV = $('#sub-bar-verificacion');
    if (sbV) sbV.classList.toggle('hidden', view !== 'verificacion');
  }
  ...
  function stopVerifCamera() {
    if (verifScanRaf) {
      cancelAnimationFrame(verifScanRaf);
      verifScanRaf = null;
    }
    if (verifScanInterval) {
      clearInterval(verifScanInterval);
      verifScanInterval = null;
    }
    if (verifMediaStream) {
      verifMediaStream.getTracks().forEach((tr) => tr.stop());
      verifMediaStream = null;
    }
    const v = $('#vf-video');
    if (v) {
      v.srcObject = null;
  ...
  function extractTokenFromScanText(raw) {
    ...
    const u = s.includes('://')
      ...
      : (() => {
          try {
            return new URL(s, 'http://local.invalid');
          } catch {
            return null;
          }
        })();
    if (u) {
      ...
      if (h) {
        const [hn, rest] = h.split('?');
        if (hn === 'verificacion' && rest) {
          return new URLSearchParams(rest).get('qr') || new URLSearchParams(rest).get('t');
        }
      }
    }
  ...
  function renderVerificacionTable(data) {
    const tb = $('#tbl-verificacion')?.getElementsByTagName('tbody')[0];
    if (!tb) return;
    tb.innerHTML = '';
      ...
      const detalle =
        b.cantidad <= 1
          ? b.restante <= 0
            ? 'Completo'
            : 'Pendiente'
          : `Ingresaron ${b.ingresados} / ${b.cantidad} — faltan ${b.restante}`;
      let acc = '<span>—</span>';
      if (b.restante > 0) {
        if (b.cantidad <= 1) {
          acc = `<button type="button" class="btn verif-one" data-bid="${b.boletaId}">Ingresó</button>`;
        } else {
          acc = `<span class="verif-delta">
            <button type="button" class="btn" data-bid="${b.boletaId}" data-add="1">+1</button>
            <input type="number" class="verif-n" data-bid="${b.boletaId}" min="1" max="${b.restante}" value="1" style="width:3.8rem" />
            <button type="button" class="btn" data-apply="${b.boletaId}">Aplicar</button>
          </span>`;
        }
      }
      tr.innerHTML = `<td>${escapeHtml(b.nombre)}</td>
        <td>${escapeHtml(b.codigoBoleta)}</td>
        <td>${b.cantidad}</td>
        <td>${b.ingresados}</td>
    ...
    tb.onclick = async (ev) => {
      ...
      if (t.classList.contains('verif-one') && bid) {
        const eventId = $('#vf-evento')?.value;
        if (!eventId) return;
        const out = await api('/api/verificacion/ingreso', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ eventId, boletaId: bid, add: 1 }),
        });
        if (out.ok) showVerifUserText(out.userText);
        else showVerifUserText(out.userText);
        await loadVerificacionList();
        return;
      }
      if (t.getAttribute('data-add') && bid) {
        const eventId = $('#vf-evento')?.value;
        if (!eventId) return;
        const out = await api('/api/verificacion/ingreso', {
          method: 'POST',
      ...
      const ap = t.getAttribute('data-apply');
      if (ap) {
        const eventId = $('#vf-evento')?.value;
        if (!eventId) return;
        const inp = tb.querySelector(`.verif-n[data-bid="${ap}"]`);
        const n = Math.max(1, parseInt(String(inp?.value || '1'), 10) || 1);
        const out = await api('/api/verificacion/ingreso', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ eventId, boletaId: ap, add: n }),
        });
        if (out.ok) showVerifUserText(out.userText);
        else showVerifUserText(out.userText);
        await loadVerificacionList();
  ...
  async function loadVerificacionList() {
    const eventId = $('#vf-evento')?.value;
    if (!eventId) {
      $('#vf-table-wrap')?.setAttribute('hidden', '');
      $('#vf-summary')?.classList.add('hidden');
      return;
    }
    const data = await api(
      '/api/verificacion/eventos/' + encodeURIComponent(eventId) + '/boletas'
    );
    const s = data.resumen;
    const su = $('#vf-summary');
    if (su) {
      su.textContent = `Personas en lista: ${s.totalPersonas} — Ingresadas: ${
        s.ingresadas
      } — Faltan por ingresar: ${s.pendientesPersonas}`;
      su.classList.remove('hidden');
    }
    const wrap = $('#vf-table-wrap');
    if (wrap) wrap.removeAttribute('hidden');
    renderVerificacionTable(data);
  }

  async function fillVerifEvents() {
    const list = await api('/api/verificacion/eventos');
    const sel = $('#vf-evento');
    if (!sel) return;
    const cur = sel.value;
    ...
    for (const e of list) {
      ...
      o.value = e.eventId;
      o.textContent = `${e.nombreProyecto || e.eventId} (${e.numBoletas} boletas)`;
      sel.appendChild(o);
    }
    if (cur && list.some((x) => x.eventId === cur)) {
      sel.value = cur;
    }
  }

  async function runVerificacionEscaneo(token) {
    const eventId = $('#vf-evento')?.value;
    if (!eventId) return;
    const out = await api('/api/verificacion/escaneo', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function loadVerificacionView() {
    if (!authState.loggedIn) return;
    await fillVerifEvents();
    const token = verificacionPendingQr;
    if (token) {
      let hint = { ok: false };
      try {
        hint = await api('/api/verificacion/qr-hint?token=' + encodeURIComponent(token));
      } catch (e) {
        console.error(e);
      }
      if (hint && hint.ok && hint.eventId) {
        const sel = $('#vf-evento');
        if (sel) sel.value = hint.eventId;
      }
      await runVerificacionEscaneo(token);
      history.replaceState(
        null,
        '',
        `${window.location.pathname}${window.location.search}#verificacion`
      );
    } else if ($('#vf-evento')?.value) {
      await loadVerificacionList();
    } else {
      const tbb = $('#tbl-verificacion')?.getElementsByTagName('tbody')[0];
  ...
  function route() {
    ...
    if (raw.includes('?')) {
      ...
      h = (name || 'inicio').trim() || 'inicio';
      if (h === 'verificacion' && rest) {
        const sp = new URLSearchParams(rest);
        verifQr = (sp.get('qr') || sp.get('t') || '').trim() || null;
      }
    } else {
      h = (raw && raw.trim()) || 'inicio';
    }
    verificacionPendingQr = verifQr;
    if (!['inicio', 'eventos', 'boletas', 'reportes', 'verificacion'].includes(h)) h = 'inicio';
    if (!authState.loggedIn && h !== 'inicio') {
      h = 'inicio';
      if (location.hash !== '#inicio') location.hash = '#inicio';
    }
    if (h !== 'verificacion') stopVerifCamera();
    ...
    if (authState.loggedIn && h === 'verificacion') {
      loadVerificacionView().catch((e) => {
        showAlert('Verificación', e.message || String(e)).catch(() => {});
      });
    }
  }
  ...
  const vfEventSel = $('#vf-evento');
  if (vfEventSel) {
    vfEventSel.addEventListener('change', () => {
      loadVerificacionList().catch((e) => console.error(e));
    });
  }
  const btnVfReporte = $('#btn-vf-reporte');
  if (btnVfReporte) {
    btnVfReporte.addEventListener('click', async () => {
      const eventId = $('#vf-evento')?.value;
      if (!eventId) {
        await showAlert('Evento', 'Seleccione un evento.');
        return;
      }
      try {
        const r = await fetch(
          '/api/verificacion/eventos/' + encodeURIComponent(eventId) + '/reporte',
        ...
        if (!r.ok) {
          ...
          throw new Error(
            (errData && errData.error) || r.statusText || 'Error al generar el reporte'
          );
        }
        const blob = await r.blob();
        const a = document.createElement('a');
        a.href = URL.createObjectURL(blob);
        a.download = `verificacion-${eventId}-${Date.now()}.xlsx`;
        a.click();
        URL.revokeObjectURL(a.href);
      } catch (e) {
        await showAlert('Reporte', e.message || String(e));
      }
    });
  }
  const btnVfPick = $('#btn-vf-pick');
  const inpVfFile = $('#vf-file-qr');
  if (btnVfPick && inpVfFile) {
    btnVfPick.addEventListener('click', () => inpVfFile.click());
  }
  if (inpVfFile) {
    inpVfFile.addEventListener('change', async (ev) => {
      const f = ev.target?.files?.[0];
      ev.target.value = '';
      if (!f) return;
      const st = $('#vf-scan-status');
      if (!('BarcodeDetector' in window)) {
        if (st) {
          st.textContent =
            'Este navegador no lee QR en imagen; use otro dispositivo o el ingreso manual en la tabla.';
        }
        return;
      }
      try {
        ...
        bmp.close();
        const t = codes && codes[0] && extractTokenFromScanText(codes[0].rawValue);
        if (!t) {
          if (st) st.textContent = 'No se leyó un QR válido.';
          return;
        }
        if (st) st.textContent = '';
        if (!$('#vf-evento')?.value) {
          await showAlert('Evento', 'Seleccione el evento antes de leer el QR.');
          return;
        }
        await runVerificacionEscaneo(t);
      } catch (e) {
        if ($('#vf-scan-status')) $('#vf-scan-status').textContent = 'Error al leer la imagen.';
        console.error(e);
      }
    });
  }
  const btnVfCam = $('#btn-vf-cam');
  if (btnVfCam) {
    btnVfCam.addEventListener('click', async () => {
      const video = $('#vf-video');
      const st = $('#vf-scan-status');
      if (verifMediaStream) {
        stopVerifCamera();
        if (st) st.textContent = '';
      ...
      if (!('BarcodeDetector' in window)) {
        if (st) st.textContent = 'Use cámara en Chrome/Edge o el ingreso manual en la lista.';
        return;
      }
      if (!$('#vf-evento')?.value) {
        await showAlert('Evento', 'Seleccione el evento antes de escanear.');
        return;
      }
      try {
        ...
        verifMediaStream = stream;
        if (video) {
          video.srcObject = stream;
          video.classList.remove('hidden');
          await video.play();
        }
        btnVfCam.setAttribute('aria-pressed', 'true');
        const BD = window.BarcodeDetector;
        const det = new BD({ formats: ['qr_code'] });
        const canvas = $('#vf-canvas');
        const ctx = canvas.getContext('2d');
        if (!ctx) return;
        verifScanInterval = window.setInterval(() => {
          if (!verifMediaStream || !video || video.readyState < 2) return;
          ...
          det
            ...
            .then((codes) => {
              if (codes && codes[0]) {
                const t = extractTokenFromScanText(codes[0].rawValue);
                if (t) {
                  stopVerifCamera();
                  if (st) st.textContent = '';
                  runVerificacionEscaneo(t).catch((e) => console.error(e));
                }
              }
            })
            .catch(() => {});
        ...
        await showAlert('Cámara', e.message || 'No se pudo abrir la cámara.');
        stopVerifCamera();
      }
    });
  }
