...
async function buildBoletaPdf({
  ...
  direccion,
  terminos,
  /** Buffer PNG (ej. de qrcode) — encima del bloque de datos, sobre la imagen */
  verificacionQrPng = null,
}) {
  ...
  if (verificacionQrPng && verificacionQrPng.length) {
    try {
      const qrEmb = await pdfDoc.embedPng(verificacionQrPng);
      const qSize = 88;
      const qM = 20;
      const qPad = 3;
      const qY = H - qM - qSize;
      page.drawRectangle({
        ...
        height: qSize + qPad * 2,
        color: rgb(1, 1, 1),
        borderColor: rgb(58 / 255, 14 / 255, 22 / 255),
        borderWidth: 0.6,
      });
      page.drawImage(qrEmb, {
        x: qM,
        y: qY,
        width: qSize,
        height: qSize,
      ...
      console.warn('No se pudo embeber QR en PDF:', e && e.message);
    }
  }
