...
function rowToBoleta(row) {
  ...
  let ingresados = Number(row[12]) || 0;
  if (ingresados < 0) ingresados = 0;
  if (cantidad > 0 && ingresados > cantidad) ingresados = cantidad;
  return {
    ...
    ingresados,
    verificacionUpdatedAt: row[13] != null && row[13] !== '' ? String(row[13]) : '',
...
async function findBoletaDataRowNumber(boletaId) {
  const rows = await getBoletaRows();
  const idx = rows.findIndex((r) => r && r[0] === boletaId);
  if (idx === -1) return null;
  return idx + 2;
}

async function updateBoletaIngresoRow(boletaId, ingresados, verificacionUpdatedAt) {
  const n = await findBoletaDataRowNumber(boletaId);
  if (n == null) throw new Error('Boleta no encontrada');
  const sheets = await getSheetsClient();
  const spreadsheetId = await getSpreadsheetId();
  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `${SHEET_BOLETAS}!M${n}:N${n}`,
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [[ingresados, verificacionUpdatedAt]] },
  });
}
