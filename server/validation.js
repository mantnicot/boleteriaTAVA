...
/** True si hoy (zona Bogotá) ya es el día de la función o posterior; la verificación en puerta solo aplica desde esa fecha. */
function isAccesoVerificacionHabilitada(fechaDmy) {
  const p = parseDMY(fechaDmy);
  if (!p) return false;
  return dmyToYYYYMMDD(p) <= todayYYYYMMDDInTz();
}
...
module.exports = {
  ...
  isAccesoVerificacionHabilitada,
  findTarifaCupo,
  validateCupo,
  soldCountForTarifa,
  normalizeTarifaKey,
